...
| `HEALTH_ENABLED` | `on` | Включить HTTP-сервер: `/health` + `/stats` (дашборд статистики) (`on`/`off`) |
...
HTTP-сервер запускается **только при `HEALTH_ENABLED=on`**, вместе с ним доступна и статистика:

| Эндпоинт | Описание |
|---|---|
| `GET /stats` | HTML-дашборд (тёмная/светлая тема, графики, автообновление по кнопке) |
| `GET /stats/api` | JSON со статистикой (можно `?from=YYYY-MM-DD&to=YYYY-MM-DD`) |

Что отслеживается:

- количество голосовых сообщений и ошибок;
- среднее время обработки (общее, STT и LLM отдельно);
- количество токенов LLM (вход/выход) и число вызовов LLM;
- суммарная и средняя длительность аудио, количество символов;
- текущие настройки: STT-провайдер/модель, устройство, LLM-модель, LLM-постпроцессинг, reasoning;
- графики: голосовых в день, активность по часам, распределение времени обработки и длительности аудио, топ пользователей, последние голосовые.

События пишутся построчно в `stats/events.jsonl` (в `.gitignore`) и агрегируются при чтении — без внешних зависимостей. Статистика накапливается между запусками; метрики «текущей сессии» считаются с момента старта процесса.
